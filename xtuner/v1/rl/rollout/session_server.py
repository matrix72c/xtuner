...
def _session_trace_attributes(session_id: Any) -> dict[str, Any]:
    if session_id is None:
        return {}
    return {"xtuner.session_id": session_id, "xtuner.rollout_id": session_id}
...
class SessionServer:
    ...
    async def _handle_request(self, request: web.Request) -> web.Response:
        ...
        with trace_span(
            ...
            parent_carrier=dict(request.headers),
        ):
    ...
    async def _handle_request_impl(self, request: web.Request) -> web.Response:
        ...
        async with ClientSession(read_bufsize=self.read_bufsize, timeout=timeout) as client:
            async with self._send_request(
                ...
                set_trace_attributes({"http.upstream.status_code": resp.status})
                if is_stream:
                    ...
                    with trace_span(
                        ...
                        attributes={
                            "xtuner.stage": "llm_read_response",
                            "session.stream": True,
                            **_session_trace_attributes(session_id),
                        },
                    ):
    ...
    @asynccontextmanager
    async def _send_request(self, client: ClientSession, *, trace_session_id=None, **kwargs):
        with trace_span(
            "session_server.send_request",
            attributes={
                "xtuner.stage": "llm_backend_request",
